...
|------|---------|------------|------------|
| `sprint.json` | Stories, waves, dependencies, verification modes, and per-story complexity hints (`"simple"` = Haiku-tier mechanical work, `"complex"` = Sonnet-tier reasoning required) — ephemeral, never committed | `/takt` command + human review | Session agent, workers |
